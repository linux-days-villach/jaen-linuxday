...
const startDate = d3.min(taskArray, data => dateFormat(data.startTime))
const endDate = d3.max(taskArray, data => dateFormat(data.endTime))
...
function makeGant(tasks: any, pageWidth: number, pageHeight: number) {
  var barHeight = 20
  var gap = barHeight + 4
  var topPadding = 75
  var sidePadding = 75
...
function drawRects(
  ...
  var innerRects = rectangles
    ...
    .attr('ry', 3)
    .attr('x', (d: any) => {
      return timeScale(dateFormat(d.startTime) || 1) + theSidePad
    })
    .attr('y', (d, i) => {
    ...
    .attr('width', (d: any) => {
      return (
        timeScale(dateFormat(d.endTime) || 1) -
        timeScale(dateFormat(d.startTime) || 1)
      )
    })
